"""Isaac Gym Preview 4 VecEnv for the Stage 1 pre-PPO locomotion baseline."""
...
class Stage1LocomotionEnv:
    ...
    def _observations(self) -> None:
        ...
        contacts = self.contact_forces[:, self.foot_indices, 2] > self.cfg.rewards.contact_threshold_n
    ...
    def step(self, actions: torch.Tensor):
        ...
        base_contact = torch.any(
            torch.linalg.vector_norm(self.contact_forces[:, self.base_indices, :], dim=-1) > 1.0,
            dim=1,
        )
        ...
        thigh_contact = torch.any(
            torch.linalg.vector_norm(self.contact_forces[:, self.thigh_indices, :], dim=-1) > 0.1,
            dim=1,
        ) if len(self.thigh_indices) else torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
